    ...
    class AssistidosMenu : Menu, IMenu, ICadastro
    {
        ...
        public bool ValidarCompleto()
        {
            ...
            if (string.IsNullOrWhiteSpace(Assistido.Usuario.Email))
                MensagemErro.AppendLine($"Nome do Usuário não pode ficar em branco");

            var Usuario = new UsuarioREP().Buscar(Assistido.Usuario);
            if (Usuario == null)
                MensagemErro.AppendLine("Usuário não localizado");
            else
            ...
            {
                var Filme = new FilmeREP().Buscar(Assistido.Filme.Nome);
                if (Filme == null)
            ...
            {
                var Serie = new FilmeREP().Buscar(Assistido.Serie.Nome);
                if (Serie == null)
        ...
        public bool ValidarSimples()
        {
            var MensagemErro = new StringBuilder();
            if (string.IsNullOrWhiteSpace(Assistido.Usuario.Email))
                MensagemErro.AppendLine($"Email do Usuário não pode ficar em branco");
            if (!string.IsNullOrWhiteSpace(MensagemErro.ToString()))
            {
                Utils.Pausar(MensagemErro.ToString());
                return false;
